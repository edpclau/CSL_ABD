...
def _shard_uids(limit, shard, nshards):
    uids = data_io.test_uids()
    if limit is not None:
        uids = uids[:limit]
    return uids[shard::nshards]   # round-robin balances long/short stays across workers


def worker(shard, nshards, limit):
    import compute_trajectories as C
    raw = data_io.load_test_raw()                       # parent already built it
    uids = _shard_uids(limit, shard, nshards)
    print(f"[worker {shard}] {len(uids)} patients", flush=True)
    C.run(uids=uids, raw=raw, out_dir=OUT_DIR, chunk_size=25,
          risk_filename=f"risk_shard{shard}.parquet")
    print(f"[worker {shard}] done", flush=True)


def parent(nshards, limit):
    ...
    for i in range(nshards):
        logf = open(OUT_DIR / f"shard{i}.log", "w")
    ...
    shard_files = sorted(OUT_DIR.glob("risk_shard*.parquet"))
    merged = pd.concat([pd.read_parquet(f) for f in shard_files], ignore_index=True)
    merged = merged.sort_values(["uid", "k"]).reset_index(drop=True)
    out = OUT_DIR / "risk_trajectories.parquet"
    merged.to_parquet(out)
    for f in shard_files:
        f.unlink()
    print(f"[parent] merged {len(shard_files)} shards -> {out}  "
...
if __name__ == "__main__":
    ...
    if args.worker is not None:
        worker(args.worker, args.nshards, args.limit)
    else:
        t0 = time.time()
        parent(args.nshards, args.limit)
